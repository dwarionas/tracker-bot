...
export function crudHandler(bot: Bot<MyContext>) {
    ...
    bot.hears('Вибрати зі збережених', onlyState(['INIT']), navigate('FROM_SAVED'), async (ctx) => {
        await ctx.reply("Збережені продукти", { reply_markup: getKeyboard(ctx.session) });
    });

    bot.hears('Назад', onlyState(["ADD_PROTEIN", "FROM_SAVED", "ADD_PRODUCT", "CHOOSE_PRODUCT"]), back(), async (ctx) => {
        await ctx.reply(ctx.session.navOptions.msg, { reply_markup: getKeyboard(ctx.session) });
    });
}